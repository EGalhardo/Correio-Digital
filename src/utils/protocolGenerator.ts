...
export function generateProtocol(
  ...
  const idValue = customId || Math.floor(100000 + Math.random() * 900000);
  ...
  // Decide province and sequence
  const province = getRandomElement(PROVINCES);
  const sequenceStr = String(Math.floor(Math.random() * 90000) + 10000).substring(0, 6);
  const year = '2026';
  
  // Formulate a classic unique protocol number conforming to examples
  // Examples: AGT-2026-LDA-000923, MINSA-2026-HSP-004221, ENDE-LUA-2026-99211
  let protocolNumber = '';
  const randPattern = Math.random();
  if (randPattern < 0.33) {
    protocolNumber = `${cleanOrg}-${year}-${province}-00${sequenceStr}`;
  } else if (randPattern < 0.66) {
    // e.g. MINSA-2026-HSP-004221
    const prefix = cleanOrg === 'HOSPITAL' ? 'MINSA' : cleanOrg;
    protocolNumber = `${prefix}-${year}-${province}-${sequenceStr}`;
  } else {
    // e.g. ENDE-LUA-2026-99211 (using LUA or active province inline)
  ...
  const qrCodeUrl = `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=AO-PROTOCOL:${protocolNumber}|ID:${internalId}|SEAL:${digitalSeal}|HASH:${documentHash}|CERT:${institutionalCertificate}|VALID:SIM&color=0f172a`;
