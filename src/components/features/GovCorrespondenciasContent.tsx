...
const prepareCorrespondence = (c: Correspondence) => {
  ...
  const history = c.history || [
    ...
    ...(status !== 'Enviada' ? [{ action: "Validação Digital pelo CDA Nacional", dateTime: `${c.date} 10:20`, user: "MINTTICS_INTEGRADOR" }] : []),
    ...(status === 'Em Análise' ? [{ action: "Início de Auditoria Administrativa", dateTime: `${c.date} 14:05`, user: "AUDITOR_MINTTICS" }] : []),
    ...(status === 'Respondida' ? [
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-all font-sans">
                    ...
                    <td className="py-4.5 px-5 font-bold">
                      <div className="space-y-1">
                        ...
                        <span className="font-sans font-extrabold text-[11px] text-slate-805 uppercase tracking-tight block truncate max-w-[200px]" title={item.sender}>
                          {item.sender}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="py-4.5 px-5 font-bold text-slate-700">
                      <div className="flex items-center gap-1.5 font-sans">
                        ...
                        <div className="space-y-0.5">
                          <span className="text-[11.5px] font-extrabold text-slate-800 tracking-tight block truncate max-w-[180px]">{item.recipient}</span>
                          <span className="text-[9.5px] font-mono text-slate-400 font-bold block">{item.destinationProvince}</span>
                        </div>
                      </div>
                    </td>
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[9px] font-black tracking-widest uppercase text-indigo-300 block leading-none">
                      Centro de Fiscalização Governamental e Interoperabilidade
                    </span>
                    <h3 className="text-base md:text-lg font-black uppercase tracking-tight text-white mt-1.5 border-0 leading-none">
                      Ofício Código: {selectedLetter.id}
                    </h3>
                    <p className="text-[10px] text-slate-400 font-bold mt-1.5 block">
                      Matéria: <span className="font-mono text-indigo-300 uppercase">{selectedLetter.category}</span> &bull; Trânsito: {selectedLetter.originProvince} &rarr; {selectedLetter.destinationProvince}
                    </p>
                  </div>
                </div>
              ...
              <div className="p-6 md:p-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-1.5 text-slate-900 border-b border-slate-100 pb-1.5">
                      ...
                      <span className="font-black text-[10px] uppercase tracking-wider block">Dados Básicos & Roteamento</span>
                    </div>

                    <div className="space-y-3.5 text-[11px] font-medium text-slate-600">
                      <div>
                        <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Entidade Emissora / Remetente</span>
                        <span className="font-black text-slate-800 block text-xs uppercase">{selectedLetter.sender}</span>
                      </div>

                      <div>
                        <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block mb-0.5 font-sans">Beneficiário Final / Destinatário</span>
                        <span className="font-black text-slate-850 block text-xs">{selectedLetter.recipient}</span>
                      </div>

                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-50">
                        <div>
                          <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Data Envio (MINTTICS)</span>
                          <span className="font-mono font-extrabold text-slate-750">{selectedLetter.sentDate}</span>
                        </div>
                        <div>
                          <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Data Receção (CDA)</span>
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-50">
                        <div>
                          <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Tempo de Resposta SLA</span>
                          <span className="font-mono font-extrabold text-slate-750">{selectedLetter.responseTime}</span>
                        </div>
                        <div>
                          <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">Prioridade de Auditoria</span>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-1.5 text-slate-900 border-b border-slate-100 pb-1.5">
                      ...
                      <span className="font-black text-[10px] uppercase tracking-wider block">Auditoria & Historial de Tráfego</span>
                    </div>
                    ...
                    <div className="space-y-3.5 max-h-[190px] overflow-y-auto pr-1 custom-scrollbar">
                      {selectedLetter.history && selectedLetter.history.map((log: any, index: number) => (
                        <div key={index} className="flex gap-2 text-[10.5px] relative pl-4 border-l border-slate-200 last:border-l-0 pb-3">
                          ...
                          <div className="space-y-0.5">
                            <p className="font-black text-slate-750 leading-none">{log.action}</p>
                    ...
                    {selectedLetter.attachments && selectedLetter.attachments.length > 0 && (
                      <div className="pt-2 border-t border-slate-100 space-y-1.5">
                        <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block">Documentos Anexados ({selectedLetter.attachments.length})</span>
                ...
                <div className="bg-slate-50 border border-slate-200/50 p-5 rounded-2xl">
                  <span className="text-[8.5px] font-black text-slate-400 uppercase tracking-widest block mb-2 leading-none">Corpo / Teor da Comunicação</span>
                  <p className="text-slate-705 leading-relaxed text-xs">{selectedLetter.body}</p>
                </div>
