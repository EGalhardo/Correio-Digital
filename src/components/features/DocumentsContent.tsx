      ...
      >
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 no-print bg-slate-50 border border-slate-200/80 p-4 rounded-[24px]">
          <button 
            ...
            className="flex items-center gap-2.5 px-5 py-2.5 bg-white border border-[#d1dbe5] rounded-full font-black text-xs text-[#384e6e] hover:bg-slate-50 transition-all shadow-sm cursor-pointer hover:scale-105 active:scale-95"
          >
            <ArrowLeft size={16} />
            <span>Voltar às Facturas</span>
          </button>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="text-lg md:text-2xl font-black text-primary leading-tight">
              {isInst ? 'Documentos Digitais' : 'Facturas Recebidas'}
            </h3>
            <p className="text-[10px] md:text-sm text-slate-600 font-black uppercase tracking-widest">
              {isInst 
                ? `${unreadTotal} novos arquivados` 
                : `${invoices.filter(i => i.status === 'Pendente').length} faturas aguardando pagamento`
              }
            </p>
          </div>
        </div>
