...
export function VoiceGuideAssistant({
  ...
  const DIALOGS = {
    welcome: {
      text: "Olá! Seja muito bem-vindo ao Correio Digital Angola, o seu novo endereço digital oficial. Eu sou o seu assistente de voz e vou ajudar-te a dar os primeiros passos. Quer que eu faça uma breve apresentação do nosso aplicativo?",
  ...
  const speak = (text: string, onEndCallback?: () => void) => {
    ...
    // Filter out visual tags for the reader
  ...
  const handleVoiceInput = (text: string) => {
    ...
    if (step === 'welcome') {
      ...
      if (text.includes('começar') || text.includes('registar') || text.includes('registo') || text.includes('iniciar') || text.includes('formulário')) {
        ...
        speak("O Correio Digital Angola é a sua caixa de correio oficial do Estado, onde receberá as suas notificações do Bilhete de Identidade, AGT e muito mais. Quer começar o seu registo agora?");
      }
    }
  };
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        ...
        {currentStep === 'offer_help' && (
          <>
            <button
              type="button"
              onClick={() => speak("O Correio Digital Angola é o portal unificado para toda correspondência oficial do Estado e segurança civil. Muito simples!")}
